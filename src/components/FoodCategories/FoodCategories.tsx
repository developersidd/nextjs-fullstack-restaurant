    ...
    return (
        <div className="">
            ...
            <div className='flex max-md:flex-wrap items-center justify-center gap-4 mt-10 md:mt-16'>
                {categories.map(cat => (
                    <button onClick={() => setCategory(() => cat)} key={cat} className={`uppercase tracking-wider px-4 py-2 text-white transition-colors  rounded-full font-bold ${ category === cat   ? "bg-primary-yellow" : "hover:text-primary-yellow"}`}> {cat}</button>
                ))}
            </div>
        </div>
    )
