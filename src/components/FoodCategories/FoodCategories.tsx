            ...
            <div className='flex max-md:flex-wrap items-center justify-center gap-4 mt-10 md:mt-16'>
                {categories.map(cat => (
                    <Link href={`?${new URLSearchParams({
                        category: cat
                    })}`} key={cat} scroll={false} className={`uppercase max-sm:text-sm tracking-wider px-4 py-2 text-white transition-colors  rounded-full font-bold ${category === cat ? "bg-primary-yellow" : "hover:text-primary-yellow"}`}> {cat}</Link>
                ))}
            </div>
        </div>
    )
